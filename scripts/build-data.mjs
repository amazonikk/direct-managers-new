...
async function main() {
  ...
  const managersChanged = JSON.stringify(previousReport?.managers ?? null) !== JSON.stringify(managers);

  const report = {
    schemaVersion: 1,
    generatedAt: managersChanged
      ? new Date().toISOString()
      : previousReport?.generatedAt || new Date().toISOString(),
    managers
  };
...
async function downloadSpreadsheet(spreadsheetId) {
  const url = `https://docs.google.com/spreadsheets/d/${spreadsheetId}/export?format=xlsx&cacheBust=${Date.now()}`;
  let lastError;
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      const response = await fetch(url, {
        ...
        cache: "no-store",
        headers: {
          "user-agent": "direct-managers-dashboard/1.1",
...
function parseManagerWorkbook(manager, workbook) {
  ...
  workbook.SheetNames.forEach((sheetName) => {
    ...
    expandWorksheetReference(worksheet);
...
function expandWorksheetReference(worksheet) {
  ...
  for (const [address, cell] of Object.entries(worksheet)) {
    if (address.startsWith("!") || !cell || (cell.v === undefined && cell.f === undefined)) continue;
