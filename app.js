(() => {
  ...
  // Дані читаються безпосередньо з Google Sheets у браузері.
  // GitHub Actions та ручне оновлення report.json для появи нових місяців не потрібні.
  const LIVE_SOURCES = [
    {
      id: "taya",
      name: "Тая",
  ...
  async function loadReport({ initial = false, silent = false } = {}) {
    ...
    if (!silent || initial) {
      setSyncStatus("loading", "Читаю Google Таблиці…");
    }
    ...
    try {
      ...
      setSyncStatus("ok", `Живі дані Google Sheets · ${loadedSheets} акаунтів · ${formatGeneratedAt(report.generatedAt)}`);
    } catch (liveError) {
      console.error("Live Google Sheets load failed:", liveError);
      ...
      try {
        ...
        selectAllAccountsForCurrentScope();
        renderAll();
        setSyncStatus("error", "Google Sheets недоступні — показано резервну копію");
        ...
        showErrorNotice(liveError.message || "Не вдалося прочитати Google Таблиці.");
        if (!state.report) showFatalError(liveError.message || "Не вдалося прочитати Google Таблиці.");
      }
    } finally {
