(() => {
  ...
  // Дані читаються через Google Apps Script, який працює від імені власника.
  // Приватні Google-таблиці не потрібно публікувати або відкривати за посиланням.
  const LIVE_SOURCES = [
    {
      id: "taya",
      name: "Тая",
      ...
      sheets: [
        "РУ-tiktok", "УКР-tiktok", "legal-tiktok", "Рум - tiktok",
        "Новинний - tiktok", "TRAKIVO - рто tiktok", "РУ-facebook",
        "РУ-inst", "УКР-inst", "legal-inst", "УЗБ - inst",
        "Новинний - inst", "ЮТУБ (ЗАГАЛЬНО)", "ТЕЛЕГРАМ"
      ]
    },
    {
      id: "kateryna",
      ...
      sheets: [
        "Ісп (cap) -tiktok", "Marocco (europe)-tiktok", "Англ (e) - tiktok",
        "УЗБ (uz) - tiktok", "Англ - INST ", "Рум - INST ",
        "ФБ-Пости", "УКР-facebook", "Вакансії", "Пошта"
      ]
    }
  ];

  const LIVE_DISPLAY_NAMES = new Map([
    ["РУ-tiktok", "TikTok — РУ"],
  ...
  async function loadReport({ initial = false, silent = false } = {}) {
    ...
    if (!silent || initial) {
      setSyncStatus("loading", "Оновлюю дані через Apps Script…");
    }
    ...
    try {
      ...
      state.selectedAccounts = new Set(
        [...previousSelection].filter((id) => validAccountIds.has(id))
      );
      ...
      renderAll();
      const loadedSheets = report.managers.reduce((sum, manager) => sum + manager.accounts.length, 0);
      setSyncStatus("ok", `Дані оновлено · ${loadedSheets} акаунтів · ${formatGeneratedAt(report.generatedAt)}`);
    } catch (liveError) {
      console.error("Apps Script load failed:", liveError);

      // Резервний варіант: показати останній збережений report.json,
      // але чітко позначити, що це не живі дані.
      try {
        ...
        state.report = report;
        const months = getAvailableMonths();
        state.month = months.includes(state.month) ? state.month : months.at(-1) || null;
        selectAllAccountsForCurrentScope();
        renderAll();
        setSyncStatus("error", "Apps Script недоступний — показано резервну копію");
        showErrorNotice(`${liveError.message || liveError} Показано останній data/report.json.`);
      } catch (fallbackError) {
        console.error(fallbackError);
        setSyncStatus("error", "Не вдалося завантажити дані");
        showErrorNotice(liveError.message || "Не вдалося отримати дані через Apps Script.");
        if (!state.report) showFatalError(liveError.message || "Не вдалося отримати дані через Apps Script.");
      }
    } finally {
  ...
  function loadAppsScriptReport() {
    return new Promise((resolve, reject) => {
      const callbackName = `__directManagersApi_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      ...
      window[callbackName] = (payload) => {
        ...
        if (!payload || typeof payload !== "object") {
          reject(new Error("Apps Script повернув порожню або некоректну відповідь."));
          return;
        }

        if (payload.error) {
          reject(new Error(`Apps Script: ${payload.error}`));
          return;
        }
      ...
      script.onerror = () => {
        ...
        window.clearTimeout(timeout);
        cleanup();
        reject(new Error("Не вдалося відкрити Google Apps Script. Перевір доступ до Web App."));
      };
  ...
  async function buildLiveReport() {
    ...
    for (const source of LIVE_SOURCES) {
      ...
      const results = await mapWithConcurrency(source.sheets, 4, async (sheetName) => {
        try {
          const rows = await loadGoogleSheetRows(source.spreadsheetId, sheetName);
          const parsed = parseLiveAccountSheet(source, sheetName, rows);
          return { sheetName, ...parsed };
        } catch (error) {
          return { sheetName, account: null, reason: error.message || String(error) };
        }
      });

      results.forEach((result) => {
        if (result.account) accounts.push(result.account);
        else ignoredSheets.push({ sheet: result.sheetName, reason: result.reason || "Аркуш не містить потрібних показників" });
      });
    ...
    const accountCount = managers.reduce((sum, manager) => sum + manager.accounts.length, 0);
    if (!accountCount) {
      throw new Error("Google Таблиці відкрилися, але не знайдено жодного аркуша з датою, чатами та номерами.");
    }
  ...
  function loadGoogleSheetRows(spreadsheetId, sheetName) {
    return new Promise((resolve, reject) => {
      const callbackName = `__directDashboard_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const script = document.createElement("script");
      let settled = false;
      ...
      const timeout = window.setTimeout(() => {
        ...
        settled = true;
        cleanup();
        reject(new Error(`Тайм-аут Google Sheets: ${sheetName}`));
      }, 25000);
      ...
      window[callbackName] = (response) => {
        ...
        if (!response || response.status === "error" || !response.table) {
          const detail = response?.errors?.map((item) => item.detailed_message || item.message).filter(Boolean).join("; ");
          reject(new Error(detail || `Google Sheets не повернув аркуш: ${sheetName}`));
          return;
        }

        const columnCount = response.table.cols?.length || 0;
        const rows = (response.table.rows || []).map((row) =>
          Array.from({ length: columnCount }, (_, columnIndex) => {
            const cell = row.c?.[columnIndex];
            if (!cell) return null;
            if (cell.v !== null && cell.v !== undefined) return cell.v;
            return cell.f !== null && cell.f !== undefined ? cell.f : null;
          })
        );
        resolve(rows);
      };
      ...
      script.onerror = () => {
        ...
        window.clearTimeout(timeout);
        cleanup();
        reject(new Error(`Не вдалося відкрити аркуш Google Sheets: ${sheetName}`));
      };
  ...
  function parseLiveAccountSheet(manager, sheetName, rows) {
    if (!rows.length) return { account: null, reason: "Порожній аркуш" };
    ...
    const columnHeaders = Array.from({ length: maxColumns }, (_, columnIndex) => {
      ...
      for (let rowIndex = 0; rowIndex < Math.min(8, rows.length); rowIndex += 1) {
        ...
        if (value !== null && value !== undefined && String(value).trim() !== "") parts.push(String(value));
      }
      return normalizeLiveText(parts.join(" "));
    });

    const dateColumn = findLiveColumn(columnHeaders, (header) => header.includes("дата")) ?? 0;
    const totalChatsColumn = findLiveColumn(columnHeaders, (header) => header.includes("загальні чати"));
    const totalNumbersColumn = findLiveColumn(columnHeaders, (header) =>
    ...
    const chatColumns = totalChatsColumn !== null
      ? [totalChatsColumn]
      : findLiveColumns(columnHeaders, (header) => [
          "к сть чатів", "кількість чатів", "к сть звернень",
          "кількість звернень", "к сть написаних", "кількість написаних"
        ].some((pattern) => header.includes(pattern)));

    if (!chatColumns.length || totalNumbersColumn === null) {
      return { account: null, reason: "Не знайдено одночасно колонки чатів і загальної кількості номерів" };
    }

    const recordsByDate = new Map();

    rows.forEach((row) => {
      const dateIso = parseLiveDateCell(row?.[dateColumn]);
      if (!dateIso) return;

      const chatValues = chatColumns.map((column) => liveNumber(row?.[column]));
      const totalNumbersValue = liveNumber(row?.[totalNumbersColumn]);
      ...
      const record = {
        date: dateIso,
        chats: Math.round(chats * 100) / 100,
        numbers: Math.round((totalNumbersValue ?? 0) * 100) / 100
      };
      const previous = recordsByDate.get(dateIso);
      if (!previous || liveActivityScore(record) > liveActivityScore(previous)) recordsByDate.set(dateIso, record);
    });

    const records = [...recordsByDate.values()].sort((a, b) => a.date.localeCompare(b.date));
    if (!records.length) return { account: null, reason: "Немає рядків із датою та показниками" };

    return {
      account: {
        id: liveStableId(manager.id, sheetName),
        sheetName,
        name: LIVE_DISPLAY_NAMES.get(sheetName) || livePrettifySheetName(sheetName),
        platform: liveDetectPlatform(sheetName),
        records
      },
      reason: null
    };
  ...
  function findLiveColumn(headers, predicate) {
    const index = headers.findIndex(predicate);
    return index === -1 ? null : index;
  }

  function findLiveColumns(headers, predicate) {
    const indexes = [];
    headers.forEach((header, index) => { if (predicate(header)) indexes.push(index); });
    return indexes;
  }

  function parseLiveDateCell(value) {
    ...
    let match = text.match(/^Date\((\d{4}),(\d{1,2}),(\d{1,2})\)$/i);
    if (match) return liveValidIso(Number(match[1]), Number(match[2]) + 1, Number(match[3]));

    match = text.match(/^(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{4})$/);
    if (match) return liveValidIso(Number(match[3]), Number(match[2]), Number(match[1]));

    match = text.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
    if (match) return liveValidIso(Number(match[1]), Number(match[2]), Number(match[3]));
    return null;
  }

  function liveValidIso(year, month, day) {
  ...
  function liveActivityScore(record) {
    return Math.abs(record.chats) + Math.abs(record.numbers) * 10;
  }

  function liveStableId(managerId, sheetName) {
    let hash = 2166136261;
    const text = `${managerId}:${sheetName}`;
  ...
  function livePrettifySheetName(sheetName) {
    return sheetName.trim().replace(/\s*[-–]\s*/g, " — ").replace(/\s+/g, " ");
  }

  function liveDetectPlatform(sheetName) {
    const normalized = normalizeLiveText(sheetName);
    ...
    if (normalized.includes("телеграм")) return "Telegram";
    if (normalized.includes("ютуб")) return "YouTube";
    if (normalized.includes("пошта")) return "Email";
    return "Інше";
  }
  ...
  function showFatalError(message) {
    const block = `<div class="empty"><div><strong>Дашборд не може завантажити дані.</strong><br>${escapeHtml(message)}<br><br>Перевірте, чи обидві Google Таблиці відкриті в режимі «Усі, хто має посилання — Читач».</div></div>`;
